...
export default class Show extends Serializable<Show> {
  ...
  constructor(showJson: Partial<Show> = {}) {
    ...
    if (showJson.stuntSheets !== undefined) {
      showJson.stuntSheets = showJson.stuntSheets.map((stuntSheet: StuntSheet): StuntSheet => {
  ...
  generateFlows(stuntSheetIndex: number): void {
    ...
    startSS.stuntSheetDots.map((startDot: StuntSheetDot): void => {
      const endDot: StuntSheetDot|undefined = startDot.dotLabelIndex === null
        ? undefined
        : endSS.stuntSheetDots.find((dot: StuntSheetDot): boolean => startDot.dotLabelIndex === dot.dotLabelIndex);

      const flow: FlowBeat[] = [];
      const continuities: BaseContinuity[] = startSS.dotTypes[startDot.dotTypeIndex];

      continuities.forEach((continuity: BaseContinuity): void => {
